...
return [
	...
	*/
	...
	{
		Route::group([
				'prefix'    => admin_uri().'/orders',
				'namespace' => 'Jiro\Extension\Order\Controllers\Admin',
			], function()
			{
				Route::get('/' , ['as' => 'admin.jiro.order.all', 'uses' => 'OrderController@index']);
				Route::post('/', ['as' => 'admin.jiro.order.all', 'uses' => 'OrderController@executeAction']);

				Route::get('grid', ['as' => 'admin.jiro.order.grid', 'uses' => 'OrderController@grid']);

				Route::get('create' , ['as' => 'admin.jiro.order.create', 'uses' => 'OrderController@create']);
				Route::post('create', ['as' => 'admin.jiro.order.create', 'uses' => 'OrderController@store']);

				Route::get('{id}'   , ['as' => 'admin.jiro.order.edit'  , 'uses' => 'OrderController@edit']);
				Route::post('{id}'  , ['as' => 'admin.jiro.order.edit'  , 'uses' => 'OrderController@update']);

				Route::delete('{id}', ['as' => 'admin.jiro.order.delete', 'uses' => 'OrderController@delete']);
			});

		Route::group([
			'prefix'    => 'orders',
			'namespace' => 'Jiro\Extension\Order\Controllers\Frontend',
		], function()
		{
	...
	*/
	...
	'permissions' => function(Permissions $permissions)
	{
		$permissions->group('order', function($g)
		{
			...
			{
				$p->label = trans('jiro/order::permissions.index');

				$p->controller('Jiro\Extension\Order\Controllers\Admin\OrderController', 'index, grid');
			});

			$g->permission('order.create', function($p)
			{
				$p->label = trans('jiro/order::permissions.create');

				$p->controller('Jiro\Extension\Order\Controllers\Admin\OrderController', 'create, store');
			});

			$g->permission('order.edit', function($p)
			{
				$p->label = trans('jiro/order::permissions.edit');

				$p->controller('Jiro\Extension\Order\Controllers\Admin\OrderController', 'edit, update');
			});

			$g->permission('order.delete', function($p)
			{
				$p->label = trans('jiro/order::permissions.delete');

				$p->controller('Jiro\Extension\Order\Controllers\Admin\OrderController', 'delete');
			});
		});
	},
